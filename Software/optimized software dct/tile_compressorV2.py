...
def compress_tile(tile_array):
    ...
    # First ensure the tile array is in a compatible format
    # The key is to make sure it's in C-contiguous memory layout with the right dtype
    tile_array_c = np.ascontiguousarray(tile_array, dtype=np.int16)
    
    # Flatten 4D array to 1D for encoding
    # We need to use ravel() instead of flatten() to avoid creating a copy
    # if the array is already contiguous
    try:
        image_array = tile_array_c.ravel()
    except:
        # Fallback to flatten() if ravel() fails
        image_array = tile_array_c.flatten()
    ...
    # Try encoding with various approaches to handle potential type issues
    try:
        # First attempt - use the array as is
        ...
        try:
            # Try with a normal Python list instead of NumPy array
            print("Trying with Python list conversion...")
            encoder = constriction.stream.stack.AnsCoder()  # Create a fresh encoder
            encoder.encode_reverse(image_array.tolist(), entropy_model)
        except TypeError as e:
            print(f"Python list conversion failed: {e}")
            
            try:
                # Try with int32 conversion
                print("Trying with int32 conversion...")
                encoder = constriction.stream.stack.AnsCoder()
                image_array_int32 = np.ascontiguousarray(image_array, dtype=np.int32)
                encoder.encode_reverse(image_array_int32, entropy_model)
            except TypeError as e:
                print(f"int32 conversion failed: {e}")
                
                # Final attempt with byte-level conversion
                print("Trying with byte-by-byte conversion (slow but compatible)...")
                encoder = constriction.stream.stack.AnsCoder()
                
                # Convert each value individually - slow but should work
                for val in reversed(image_array):
                    encoder.encode(val, entropy_model)
