...
public:
	...
	void encode(uint8_t *symbol_arr, size_t num_symbols) {
		...
		for(int i = 0; i < num_symbols; i++) {
			...
			//Encode symbol the same way the hardware does it
			uint32_t range = upper - lower + 1;

			//Get probability range inverse to emulate the hardware pipeline division approximation
			uint32_t range_inv = (uint32_t) 0x10000 / (uint32_t) bounds.range;

			//Calculate new upper and lower bound partial data
