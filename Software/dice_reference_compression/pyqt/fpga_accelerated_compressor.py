...
def process_image_channels(
    ...
    tile_id = np.zeros(
        (n_tiles_x, n_tiles_y), dtype=np.int16,
    )
    ...
    # print fpga_tile_id for debugging

    print("\nTile ID Array:")
    print(fpga_tile_id)

    #block_ids = fpga_tile_id.reshape(-1)  # Flatten the tile ID array for easier processing
    ...
    if EN_TILE_REPLACEMENT:
        for row in range(n_tiles_x):
            for col in range(n_tiles_y):
                ...
                #current_tile = fpga_tile_id[row][col]
